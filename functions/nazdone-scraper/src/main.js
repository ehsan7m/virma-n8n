...
function sizeOrderValue(labelRaw) {
  ...
  if (!isNaN(num)) return 1000 + num; // سایز عددی
  ...
  const yearMatch = label.match(/(\d+)\s*سال/); // 5 سال
...
async function scrapeOne(page, productUrl, log) {
  ...
  const data = await page.evaluate(() => {
    ...
    // تصاویر (یونیک، حذف ابعاد بندانگشتی)
    const imgs = new Set();
    ...
    // تلاش برای پیداکردن JSON در اسکریپت‌ها (sizes/colors/stockId)
    function digForJson() {
    ...
    // اگر colors_flat نبود، از روی سایزها بساز
...
export default async (context) => {
  ...
  log(`Nazdone scraper start → mode=${mode} urls=${productUrls.length} limit=${limit}`);
  ...
  try {
    if (mode === 'category' && categoryUrl) {
      ...
      log(`Found ${links.length} links, scraping ${take.length}...`);

      for (const link of take) {
        const p = await scrapeOne(page, link, log);
        out.push(p);
      }
      ...
      error('Invalid input (need mode/productUrls or mode/categoryUrl)');
      try { await browser.close(); } catch {}
      return res.json({ ok: false, error: 'Invalid input. Provide {mode:"product", productUrls:[...]} or {mode:"category", categoryUrl:"...", limit:N}' }, 400);
    }
  } catch (e) {
